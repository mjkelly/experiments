...
parser.add_option('--amz-key-id', dest='key_id', help='Amazon API key ID.')
parser.add_option('--amz-key-secret', dest='key_secret',
                  help='Amazon API key secet value.')
parser.add_option('--domain', dest='domain', help='Domain name to update.')
parser.add_option('--zone-id', dest='zone_id',
                  help='Amazon zone ID containing domain name.')
parser.add_option('--ip', dest='ip', help='New IP for domain name.')
...
print "old_ip=%s, old_ttl=%s" % (old_ip, old_ttl)
if old_ip is None:
  raise RuntimeError('Previous IP for A record does not exist or is not parseable.')

if old_ip == new_ip:
  print 'Old IP %s is same as new IP. We can stop.' % old_ip
  sys.exit(0)
